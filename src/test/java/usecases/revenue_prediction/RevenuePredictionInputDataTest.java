...
class RevenuePredictionInputDataTest {
    ...
    @Test
    void testWithZeroIntervalLength() {
        RevenuePredictionInputData inputData = new RevenuePredictionInputData("Average Model", 0, "day");
        assertEquals("Average Model", inputData.getModelName(), "Model name should be valid");
        assertEquals(0, inputData.getIntervalLength(), "Interval length should be zero");
        assertEquals("day", inputData.getIntervalName(), "Interval name should be valid");
    }

    @Test
    void testWithNegativeIntervalLength() {
        RevenuePredictionInputData inputData = new RevenuePredictionInputData("Average Model", -5, "day");
        assertEquals("Average Model", inputData.getModelName(), "Model name should be valid");
        assertEquals(-5, inputData.getIntervalLength(), "Interval length should be negative");
        assertEquals("day", inputData.getIntervalName(), "Interval name should be valid");
    }
}
